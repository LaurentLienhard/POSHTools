Function Get-RdsServerList {
    ...
    Process {
        $Result = (Get-ADComputer @ADParams | Where-Object { $_.name -match "^SRV-RDS[\d].*|^TS[\d].*|^SRV-RDSCTA-[\d].*|^SRV-RDSCTAHM-[\d].*|^SRV-RDSCTACH-[\d].*|^SRV-RDSHM-[\d].*|^SRV-REMOTEAPP[\d].*" } | Select-Object Name).Name
    }
